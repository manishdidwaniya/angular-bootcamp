...
@Component({
  ...
  template: `
    <div
      class="min-h-screen flex items-center justify-center bg-surface-900 px-4"
    >
      <div class="w-full max-w-md">
        <!-- Logo -->
        <div class="text-center mb-8">
          <div
            class="w-16 h-16 rounded-2xl bg-brand-300 flex items-center justify-center mx-auto mb-4"
          >
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
          <div>
            <label for="login-email" class="block text-sm font-medium mb-1.5"
              >Email</label
            >
            <input
              id="login-email"
          ...
          <div>
            <label for="login-password" class="block text-sm font-medium mb-1.5"
              >Password</label
            >
            <input
              id="login-password"
          ...
          @if (error()) {
            <div
              class="text-red-400 text-sm bg-red-400/10 border border-red-400/20 rounded-lg px-4 py-2"
            >
              {{ error() }}
            </div>
          }
        ...
        <p class="text-center text-sm text-[var(--text-muted)] mt-6">
          ...
          <a routerLink="/auth/register" class="text-brand-300 hover:underline"
            >Create one</a
          >
        </p>
      </div>
